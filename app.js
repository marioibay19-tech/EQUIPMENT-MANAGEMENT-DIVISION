...
async function enterApp(){
  ...
  if(error){
    ...
    const roleMap = { staff:'Staff', supervisor:'Supervisor', driver:'Driver', end_user:'End-user' };
    role = roleMap[profile.role] || 'Staff';
  }
  ...
  if(canManage()){
    document.getElementById('requestsNavLink').textContent = 'All Requests';
    document.getElementById('requestsPageTitle').textContent = 'All Requests';
    document.getElementById('requestsPageSub').textContent = 'Full request log';
  }else{
    document.getElementById('requestsNavLink').textContent = 'My Requests';
    document.getElementById('requestsPageTitle').textContent = 'My Requests';
    document.getElementById('requestsPageSub').textContent = 'Requests you have submitted';
  }
...
function renderAllRequests(){
  ...
  body.innerHTML = rows.map(r=>{
    ...
    const statusCell = canManage()
      ? `<select class="status ${STATUS_CLASS[r.status]||''}" data-id="${r.id}" onchange="updateStatus(this)">${options}</select>`
...
// ---------- Spare parts: view/edit modal ----------
let editingSpareId = null;

function openSpareModal(id){
  const r = allSpareParts.find(x=>x.id===id);
  if(!r) return;
  editingSpareId = id;
  document.getElementById('spModalMsg').innerHTML = '';
  document.getElementById('spEditPlate').value = r.plate || '';
  document.getElementById('spEditDpcn').value = r.dpcn || '';
  document.getElementById('spEditDept').value = r.department || '';
  document.getElementById('spEditEndUser').value = r.end_user || '';
...
async function saveSpareEdit(){
  ...
  const btn = document.getElementById('spEditSaveBtn');
  btn.disabled = true; btn.textContent = 'Saving…';

  const { error } = await sb.from('spare_parts_requests').update({
    plate: normalizePlate(document.getElementById('spEditPlate').value),
    dpcn: document.getElementById('spEditDpcn').value.trim() || null,
    department: document.getElementById('spEditDept').value,
    end_user: document.getElementById('spEditEndUser').value.trim() || null,
    date_request: document.getElementById('spEditDateRequest').value || null,
    requests: document.getElementById('spEditRequests').value.trim(),
    remarks: document.getElementById('spEditRemarks').value.trim() || null,
    updated_at: new Date().toISOString()
